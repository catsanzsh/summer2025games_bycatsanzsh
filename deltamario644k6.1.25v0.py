...
class Mario64(Entity):
    ...
    def update(self):
        ...
        ground_ray = raycast(self.world_position + Vec3(0, 0.1, 0), self.down,
                             distance=0.7, ignore=[self] + self.children)
        if ground_ray.hit and self.velocity_y <= 0:
            self.y = ground_ray.world_point.y + 0.01
            self.velocity_y = 0
            self.grounded = True
            self.jump_count = 0
        else:
...
# Ground & terrain
Entity(model='plane', collider='mesh', scale=100, texture='white_cube',
       texture_scale=(50, 50), color=color_grass_green)
# Hills and platforms
